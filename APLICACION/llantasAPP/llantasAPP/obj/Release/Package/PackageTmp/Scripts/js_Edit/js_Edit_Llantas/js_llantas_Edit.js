...
var $cajas = $(".caja");
var $llantas = $(".llanta");
var _contadorMontadas = 0;
var _maximasMontadas = 0;
...
$("#fomrAddllanta").on("submit", function(event) {
  ...
  if (estado != false) {
    if (_contadorMontadas < _maximasMontadas) {
      var nroLlanta = $("#comboDisponibles")
        .val()
        .toUpperCase();
      add_nuevallanta(nroLlanta);
      _contadorMontadas = _contadorMontadas + 1;
    }
  }
});
...
function llantasMaximas() {
  var NroEjes = $("#NroEjes").text();
  var t_vehiculo = $("#t_vehiculo").text();
  switch (NroEjes) {
    case "1":
      _maximasMontadas = 4;
      break;
  ...
  console.log("monto maximo " + _maximasMontadas);
}
...
function SentidoLlantas() {
  arraySentido = [];
  var datosLlantas = $('[tag="preload"]');
  for (let i = 0; i < datosLlantas.length; i++) {
    const idLlanta = $(datosLlantas[i]).attr("Id");
    const sentido = $("#" + idLlanta + " .imgSentido").attr("tag");
    var llanta = {
      LLANTA: idLlanta,
      SENTIDO: sentido
    };
    arraySentido.push(llanta);
  }
}
...
function montarLlanta(_placa, _llanta, _grupo, _kmMEdida, Operacion) {
